    ...
    private static void setupSparkServer() {
        EmbeddedServers.add(JETTY,
                (routeMatcher, staticFilesConfiguration, hasMultipleHandler) -> {
                    ...
                    final MatcherFilter matcherFilter = new MatcherFilter(routeMatcher, staticFilesConfiguration, false, hasMultipleHandler);
                    matcherFilter.init(null);

                    return new EmbeddedJettyServer((int maxThreads, int minThreads, int threadTimeoutMillis) -> new Server(), new JettyHandler(decorateWithMLAuth(matcherFilter)));
                }
        );
    }
    ...
    private static Filter decorateWithMLAuth(final Filter decoratedFilter) {
        return new Filter() {
            ...
            public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain)
                ...
                mlAuthFilter.doFilter(request, response, (req, res) -> decoratedFilter.doFilter(req, res, chain));
            }
