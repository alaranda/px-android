...
public final class SimpleValidatorHelper {
    ...
    public static Validator<String> isNumber(final String param) {
        return SimpleValidator.from((s) -> NumberUtils.isNumber(s), format("%s must be number.", param));
    }
    ...
    static Validator<BigDecimal> isBigDecimalPositive(final String param) {
        return SimpleValidator.from(number -> number != null && number.signum() == 1, format(POSITIVE_ERROR_MSGE, param));
    }
}