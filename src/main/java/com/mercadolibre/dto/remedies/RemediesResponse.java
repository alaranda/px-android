...
public class RemediesResponse {
    ...
    public String toLog(final RemediesResponse remediesResponse) {
        ...
        if (null  != remediesResponse.getCvv()) {
           appendTitleAndMessage(stringBuilder, remediesResponse.getCvv().getTitle(), remediesResponse.getCvv().getMessage());
           stringBuilder.append(String.format("field_setting", remediesResponse.getCvv().getFieldSetting().toString()));
        }
        if (null != remediesResponse.getCallForAuth()){
            appendTitleAndMessage(stringBuilder, remediesResponse.getCallForAuth().getTitle(), remediesResponse.getCallForAuth().getMessage());
        }
        if (null != remediesResponse.getHighRisk()) {
            appendTitleAndMessage(stringBuilder, remediesResponse.getHighRisk().getTitle(), remediesResponse.getHighRisk().getMessage());
