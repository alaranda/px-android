...
public class PublicKeyAPITest extends RestClientTestBase {
    ...
    @Test
    public void getPublicKey_fail() throws IOException, ApiException, ExecutionException, InterruptedException {
        MockPublicKeyAPI.getPublicKey(PUBLIC_KEY_FAIL,
                HttpStatus.SC_INTERNAL_SERVER_ERROR,
                IOUtils.toString(getClass().getResourceAsStream("/publicKey/invalidPK.json"))
        );
        CompletableFuture<Either<PublicKey, ApiError>> futurePublicKey = publicKeyAPI.getAsyncById(context, PUBLIC_KEY_FAIL);
    ...
    @Test
    public void getPublicKey_ok() throws IOException, ApiException, ExecutionException, InterruptedException {
        MockPublicKeyAPI.getPublicKey(PUBLIC_KEY_OK,
                HttpStatus.SC_OK,
                IOUtils.toString(getClass().getResourceAsStream(String.format("/publicKey/%s.json", PUBLIC_KEY_OK)))
        );

        CompletableFuture<Either<PublicKey, ApiError>> futurePublicKey = publicKeyAPI.getAsyncById(context, PUBLIC_KEY_OK);
