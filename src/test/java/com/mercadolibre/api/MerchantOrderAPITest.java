...
public class MerchantOrderAPITest extends RestClientTestBase {
    ...
    public void createMerchantOrder_isOk() throws IOException, ApiException {
        MockMerchantOrderAPI.createMerchantOrder(COLLECTOR_ID, HttpStatus.SC_OK,
                IOUtils.toString(getClass().getResourceAsStream("/merchantOrders/merchantOrderResponse.json")));

        MerchantOrder merchantOrder = new MerchantOrder.Builder().withMarketplace("MLA").buildMerchantOrder();
    ...
    public void createMerchantOrder_isFail() throws IOException, ApiException {
        MockMerchantOrderAPI.createMerchantOrder(COLLECTOR_ID, HttpStatus.SC_BAD_REQUEST,
                IOUtils.toString(getClass().getResourceAsStream("/merchantOrders/invalidMerchantOrderResponse.json")));
        MerchantOrder merchantOrder = new MerchantOrder.Builder().withMarketplace("MLA").buildMerchantOrder();
        final Either<MerchantOrder, ApiError> merchantOrderResponse =
                merchantOrderAPI.createMerchantOrder(context, merchantOrder, COLLECTOR_ID);
