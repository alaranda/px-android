...
public class CongratsServiceTest {
    ...
    public void getPointsAndDiscounts_validParams_crossSellingDiscountsAndPoints() throws IOException {
        ...
        MockLoyaltyApi.getAsyncPoints(getDefaultCongratsRequestMock(), HttpStatus.SC_OK, IOUtils.toString(getClass().getResourceAsStream("/loyalty/loyalResponseOk.json")));

        MockMerchAPI.getAsyncCrosselingAndDiscount(getDefaultCongratsRequestMock(), HttpStatus.SC_OK, IOUtils.toString(getClass().getResourceAsStream("/merch/merchResponseCrossSellingAndDiscounts.json")));

        final Congrats congrats = congratsService.getPointsAndDiscounts(CONTEXT_ES, getDefaultCongratsRequestMock());
    ...
    public void getPointsAndDiscounts_validUserAgentAndroid_crossSellingAndDiscounts() throws IOException {
        ...
        final CongratsRequest congratsRequest = new CongratsRequest(USER_ID_TEST, CLIENT_ID_TEST, Site.MLA.name(), PAYMENT_IDS_TEST, PLATFORM_TEST_MP, invalidUserAgent, DENSITY, PRODUCT_ID, CAMPAIGN_ID_TEST, FLOW_NAME, false, null);
